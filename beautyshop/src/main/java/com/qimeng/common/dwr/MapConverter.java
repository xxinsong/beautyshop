        ...
        try
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                if(valType == String.class){
	                if( valIv.getValue().startsWith(ProtocolConstants.INBOUND_MAP_START) &&  valIv.getValue().endsWith(ProtocolConstants.INBOUND_MAP_END)){
	                	valType = Map.class;
	                }else if( valIv.getValue().startsWith(ProtocolConstants.INBOUND_ARRAY_START) &&  valIv.getValue().endsWith(ProtocolConstants.INBOUND_ARRAY_END)){
	                	valType = List.class;
	                }
                }
